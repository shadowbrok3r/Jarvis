...
final class JarvisHubWebSocketClient: @unchecked Sendable {
    ...
    func connect(webSocketURL: URL, hubBearerToken: String) {
        stop()
        let t = session.webSocketTask(with: webSocketURL)
        task = t
        JarvisIOSLog.recordIronclaw("hub WS: connecting \(webSocketURL.absoluteString)")
        t.resume()
        sendEnvelope(type: "module:authenticate", data: ["token": hubBearerToken], source: moduleName)
        sendEnvelope(
